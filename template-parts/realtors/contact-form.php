...
$hide_form_fields = yani_option('hide_agency_agent_contact_form_fields');

if ( is_singular( 'yani_agent' ) ) {
    global $post;
    $target_email = get_post_meta( $post->ID, 'yani_agent_email', true );
    $agent_id = $post->ID;
    $agent_type  = 'agent_info';
    $source_link = get_permalink();
    $name = get_the_title();

    $form_shortcode = yani_option('contact_form_agent_detail');

} else if ( is_singular( 'yani_agency' ) ) {
    global $post;
    $target_email = get_post_meta( $post->ID, 'yani_agency_email', true );
    $name = get_the_title();
    ...
    $form_shortcode = yani_option('contact_form_agency_detail');
    
} else if ( is_author() ){
    global $current_author, $current_author_meta;
    $target_email = $current_author->user_email;
    $name = $current_author->display_name;
    $image_url = $current_author_meta['yani_author_custom_picture'][0];
                    ...
                    <div class="agent-details">
                        <div class="d-flex align-items-center">
                            ...
                            <ul class="agent-information list-unstyled">
                                <li class="agent-name">
                                    <i class="yani-icon icon-single-neutral mr-1"></i> <?php echo esc_attr($name); ?>
                                </li>
                            </ul>
