<?php
$currency_symbol = yani_option( 'currency_symbol' );
$where_currency = yani_option( 'currency_position' );
$price_listing_submission = yani_option('price_listing_submission');
$price_featured_submission = yani_option('price_featured_listing_submission');
        ...
        <div class="membership-package-order-detail">
            ...
            <ul class="list-unstyled mebership-list-info">
                <?php if( !empty( $upgrade_id ) ) {
                    ...
                    $prop_featured = get_post_meta($upgrade_id, 'yani_featured', true);
                    if ($prop_featured != 1) { ?>
                        <li>
                            <i class="yani-icon icon-check-circle-1 mr-2 primary-text"></i> 
                            <?php esc_html_e('Featured Fee', _yani_theme()->get_text_domain()); ?>
            ...
            <?php } else { ?>
                ...
                <?php if( !empty( $prop_id ) ) {
                    $prop_featured = get_post_meta($prop_id, 'yani_featured', true);
                    if ($prop_featured != 1) {
                        ?>
                        <li>
                            <i class="yani-icon icon-check-circle-1 mr-2 primary-text"></i>
                        ...
                        <li>
                            <?php esc_html_e('Make Featured', _yani_theme()->get_text_domain()); ?>
                            <strong>
