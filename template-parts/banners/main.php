...
if(empty($post_id) && !_yani_taxonomy()->is_tax()) {
    return;
}
...
if(_yani_taxonomy()->is_tax()) {
    if(_yani_theme()->get_option('tax_show_map')) {
        $banner_type = 'property_map';
    }
} else {
    $banner_type = get_post_meta($post->ID, 'yani_header_type', true);
}
...
if( _yani_template()->is_search_needed() ) {
    ...
    if( _yani_template()->is_transparent_logo() ) {
