<?php
$currency_switcher_enable = yani_option('currency_switcher_enable');
$is_multi_currency = yani_option('multi_currency');
if( $currency_switcher_enable != 0 && $is_multi_currency != 1 ) {
    if (class_exists('FCC_Rates')) {

        $supported_currencies = yani_get_list_of_supported_currencies();

        if (0 < count($supported_currencies)) {

            $current_currency = yani_get_wpc_current_currency();
            ?>
