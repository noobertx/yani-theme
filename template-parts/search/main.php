...
if( wp_is_mobile() ) {
    $search_sticky = _yani_theme()->get_option('mobile-search-sticky');
} else {
    $search_sticky = _yani_theme()->get_option('main-search-sticky');
}
...
if( _yani_search()->adv_search_visible() ) {
...
if( (!_yani_theme()->get_option('single_prop_search') && is_singular('property')) || (!_yani_theme()->get_option('single_agent_search') && is_singular('yani_agent')) || (!_yani_theme()->get_option('single_agent_search') && is_singular('yani_agency')) || ( !_yani_theme()->get_option('is_tax_page', 1) && is_tax() ) ) {
	return;
}
