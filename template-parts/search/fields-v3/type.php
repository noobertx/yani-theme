<div class="btn-group">
	<button type="button" class="btn btn-light-grey-outlined" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<?php echo yani_option('srh_type', 'Type'); ?>
	</button>
		...
		if( taxonomy_exists('property_type') ) {
		    ...
		    if ( !empty($prop_type) && ! is_wp_error( $prop_type ) ) {
		        ...
		        foreach ($prop_type as $type):
		            ...
		            echo '<input class="'.yani_get_ajax_search().'" name="type[]" type="checkbox" '.$checked_type.' value="' . esc_attr( $type->slug ) . '">';
                    ...
                    if (!empty($get_child)) {
                        ...
                        foreach($get_child as $child) {
                            ...
                            echo '<label class="control control--checkbox hz-checkbox-'.$count.'">';
                                echo '<input class="'.yani_get_ajax_search().'" name="type[]" type="checkbox" '.$checked_type2.' value="' . esc_attr( $child->slug ) . '">';
