    ...
    <div class="dashboard-header-wrap">
        <div class="d-flex align-items-center">
            <div class="dashboard-header-left flex-grow-1">
                ...
                <h1><?php esc_html_e('Inquiries', _yani_theme()->get_text_domain()); ?></h1>  

                <?php } elseif($events == 'active') { ?>
                <h1><?php esc_html_e('Events', _yani_theme()->get_text_domain()); ?></h1>

                <?php } elseif($viewed == 'active') { ?> 
                <h1><?php esc_html_e('Listings Viewed', _yani_theme()->get_text_domain()); ?></h1> 

                <?php } elseif ($searches == 'active') { ?>
                <h1><?php esc_html_e('Saved Searches', _yani_theme()->get_text_domain()); ?></h1> 
                
                <?php } elseif ($notes == 'active') { ?>
                <h1><?php esc_html_e('Notes', _yani_theme()->get_text_domain()); ?></h1> 
               <?php } ?>    
            </div><!-- dashboard-header-left -->
            <div class="dashboard-header-right">
                <?php if( $enquires == 'active' ) { ?>
                <a href="#" class="btn btn-primary open-close-enquiry-panel">
                    <?php esc_html_e('Add New Inquiry', _yani_theme()->get_text_domain()); ?>
                </a>
                <?php } elseif( $events == 'active' ) { ?>
                <a href="#" class="btn btn-primary open-close-event-panel">
                    <?php esc_html_e('Add New Event', _yani_theme()->get_text_domain()); ?>
                </a>
                <?php } ?>
