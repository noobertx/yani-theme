...
// $yani_map_system = yani_get_map_system();
$yani_map_system = "";
...
if (_yani_post()->can_be_editted()) {
    $lat_lng = _yani_field()->get_field_meta('property_location');
...
<div id="location" class="dashboard-content-block-wrap <?php echo esc_attr($is_multi_steps);?>">
	<h2><?php echo _yani_theme()->get_option('cls_location', 'Location'); ?></h2>
	...
	<h2><?php echo _yani_theme()->get_option('cls_map', 'Map'); ?></h2>
	<div class="dashboard-content-block">
		<div class="row">
			<div class="col-md-6 col-sm-12">
				<div class="form-group dashboard-map-field">
					<label><?php echo _yani_theme()->get_option('cl_drag_drop_text', 'Drag and drop the pin on map to find exact location'); ?></label>
				...
				<button id="find_coordinates" type="button" class="btn btn-primary btn-full-width"><?php echo _yani_theme()->get_option('cl_ppbtn', 'Place the pin in address above'); ?></button>
				<a id="reset" href="#" style="display:none;"><?php esc_html_e('Reset Marker', _yani_theme()->get_text_domain());?></a>
