...
$user_status = '<span class="text-danger"><i class="yani-icon icon-single-neutral-circle mr-1"></i>'.esc_html__('Offline', _yani_theme()->get_text_domain()).'</span>';

if ( isset( $_GET['seen'] ) && $_GET['seen'] == 1 ) {
	yani_update_message_status( $current_user_id, $thread_id );
}
...
$thread_author = $yani_thread->sender_id;

if ( $thread_author == $current_user_id ) {
	$thread_author = $yani_thread->receiver_id;
} 
...
$user_custom_picture =  get_the_author_meta( 'yani_author_custom_picture' , $thread_author );

if ( empty( $user_custom_picture )) {
	$user_custom_picture = get_template_directory_uri().'/img/profile-avatar.png';
}

if ( yani_is_user_online( $thread_author ) ) {
	$user_status = '<span class="text-success ml-2"><i class="yani-icon icon-single-neutral-circle mr-1"></i>'.esc_html__('Online', _yani_theme()->get_text_domain()).'</span>';
}
?>
        ...
        <div class="d-flex">
            ...
            <div class="message-reply-user">
                <span class="mr-2"><?php echo ucfirst( $thread_author_display_name ); ?> 
                    ...
                    <br>
                    <strong><?php echo get_the_title( $yani_thread->property_id ); ?></strong>
                </span>
            </div><!-- message-reply-user -->
        </div><!-- d-flex -->
...
<div class="dashboard-content-block">
    ...
    <div class="message-reply-wrap">
        <div class="d-flex">
            ...
            <div class="message-reply-user mr-3">
                <?php
				$current_user_picture =  get_the_author_meta( 'yani_author_custom_picture' , $current_user_id );
