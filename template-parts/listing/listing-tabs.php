<?php
global $post, $total_listing_found, $yani_local, $listings_tabs;
$listing_page_link = get_permalink( $post->ID );
$listings_tab_1 = get_post_meta( $post->ID, 'yani_listings_tab_1', true );
$listings_tab_2 = get_post_meta( $post->ID, 'yani_listings_tab_2', true );

$tab1_link = add_query_arg( 'tab', $listings_tab_1, $listing_page_link );
$tab2_link = add_query_arg( 'tab', $listings_tab_2, $listing_page_link );

$tab_1 = _yani_terms()->get_term_by( 'slug', $listings_tab_1, 'property_status' );
$tab_2 = _yani_terms()->get_term_by( 'slug', $listings_tab_2, 'property_status' );
...
$property_label = _yani_theme()->get_option('cl_property', 'Property');
if( $total_listing_found > 1 ) {
   $property_label = _yani_theme()->get_option('cl_properties', 'Properties'); 
}
?>
<div class="listing-tabs flex-grow-1">
    ...
    if($listings_tabs != 'disable') {
        ...
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link <?php echo esc_attr( $tab_all ); ?>" href="<?php echo esc_url( $listing_page_link ); ?>"><?php echo esc_html__('All', _yani_theme()->get_text_domain()); ?></a>
            </li>
