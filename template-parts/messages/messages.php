...
$yani_threads = $wpdb->get_results( $message_query );
?>

<?php if ( sizeof( $yani_threads ) != 0 ) { ?>
<table class="dashboard-table table-lined responsive-table">
    ...
    <?php 
	foreach ( $yani_threads as $thread ) { 
	...
	$tabel = $wpdb->prefix . 'yani_thread_messages';
	$sender_id = $thread->sender_id;
	$thread_id = $thread->id;
	...
	$user_custom_picture =  get_the_author_meta( 'yani_author_custom_picture' , $sender_id );
	...
	$thread_link = yani_get_template_link_2('template/user_dashboard_messages.php');
		...
		<td data-label="<?php esc_html_e( 'Actions', 'houzez' ); ?>">
			<div class="dropdown property-action-menu">
				<button class="btn btn-primary-outlined dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					<?php esc_attr_e('Actions', _yani_theme()->get_text_domain()); ?>
				</button>
				<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
					<a class="dropdown-item" href="<?php echo esc_url( $thread_link ).'#message-'.intval($last_message->id); ?>"><?php esc_attr_e('Reply', _yani_theme()->get_text_domain());?></a>
					<a class="yani_delete_msg_thread dropdown-item" href="#" data-thread-id="<?php echo intval($thread_id); ?>" data-sender-id="<?php echo intval($sender_id); ?>" data-receiver-id="<?php echo intval($receiver_id); ?>"><?php esc_attr_e('Delete', _yani_theme()->get_text_domain()); ?></a>
				</div>
			</div>
		</td>
	</tr>
...
<div class="dashboard-content-block">
    <?php echo esc_html__("You don't have any message.", _yani_theme()->get_text_domain()); ?>
